<x-layout color="bg-darkblue-500">
    ...
    <main class="sm:ml-64 sm:mt-14 my-16 flex flex-col">
        <div class="flex flex-col px-6 sm:px-10 pt-4 sm:pt-6">
            @php
                $isBookmarked = Auth::check() && $item->bookmarks()->where('user_id', Auth::id())->exists();
            @endphp
            ...
            <div class="flex flex-col sm:flex-row mt-4 mb-7">
                ...
                <div class="flex flex-col ml-0 sm:ml-4 mt-4 sm:mt-0">
                    ...
                    <div class="flex mt-2 items-center">
                        ...
                        <a href="{{ route('item', ['category' => $item->category->id]) }}"
                    ...
                    <div x-data="{ open: @if ($errors->any()) true @else false @endif, bookmarkOpen: false, unbookmarkOpen: false }" class="flex mt-2 h-10">
                        <!-- Borrow Button -->
                        ...
                        <!-- Bookmark Button -->
                        <button @click="{{ $isBookmarked ? 'unbookmarkOpen' : 'bookmarkOpen' }} = true"
                            class="ml-2 px-3 {{ $isBookmarked ? 'bg-indigo-500 hover:bg-indigo-700' : 'bg-darkblue-200 hover:bg-darkblue-300' }} rounded-lg">
                                ...
                                <form action="{{ route('request.store') }}" method="post">
                                    ...
                                    <div x-data="{ minAmount: 1, maxAmount: {{ $item->amount }}, amount: null }" class="mt-2 flex flex-col">
                                        ...
                                        <input
                                            ...
                                            @input="if ($event.target.value > maxAmount) $event.target.value = maxAmount; if ($event.target.value < minAmount) $event.target.value = minAmount">
                                        @error('total_request')
                        ...
                        <!-- Modal for Bookmark Confirmation -->
                        <div x-show="bookmarkOpen" x-transition
                            class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" x-cloak>
                            <div @click.outside="bookmarkOpen = false"
                                class="bg-darkblue-300 rounded-lg p-6 w-[70%] sm:w-[400px]">
                                <div class="flex justify-between">
                                    <h2 class="text-xl text-slate-200 mb-4">Bookmark {{ $item->name }} ?</h2>
                                    <span @click="bookmarkOpen = false" class="text-slate-200">
                                        <i class="fa fa-x"></i>
                                    </span>
                                </div>
                                <div class="text-slate-500">
                                    <p>This action will added this item to bookmark</p>
                                </div>
                                <div class="flex justify-end mt-6">
                                    <form action="{{ route('bookmark.store', $item->id) }}" method="post">
                                        @csrf
                                        <button type="submit"
                                            class="flex px-3 py-2 gap-x-2 text-sm tracking-wide text-white capitalize transition-colors duration-200 transform bg-indigo-500 rounded-md">
                        ...
                        <div x-show="unbookmarkOpen" x-transition
                            ...
                            <div @click.outside="unbookmarkOpen = false"
                                ...
                                <div class="text-slate-500">
                                    <p>This action will remove this item from bookmark</p>
                                </div>
                                <div class="flex justify-end mt-6">
                                    <form action="{{ route('bookmark.destroy', $item->id) }}" method="post">
                                        @csrf
                                        @method('DELETE')
            ...
            <div class="flex flex-col items-start justify-start mt- rounded-xl bg-darkblue-400 h-[250px] sm:h-[300px]">
                <div class="flex flex-col w-full overflow-y-auto no-scrollbar">
                    @forelse ($lendings as $lending)
                        <div
                            class="px-4 py-5 bg-darkblue-300 border-b border-darkblue-200 flex justify-between items-center 
                            ...
                            <div class="flex items-start space-x-2">
                                ...
                                <div class="flex flex-col">
                                    ...
                                    <div class="flex flex-col sm:flex-row sm:items-center sm:space-x-2">
                                        <p class="text-xs mt-1 text-slate-500">
                                            Total Borrowed: <span
                                                class="text-indigo-400">{{ $lending->total_request }}</span>
                                        </p>
                                        <p class="text-xs mt-1 text-slate-500">
                                            Return Date: <span
